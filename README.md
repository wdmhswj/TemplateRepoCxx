...
- [ ] 多线程爬取：支持设置最大线程数，并行处理多个URL
- [ ] URL管理：自动处理URL规范化、去重和队列管理
- [ ] robots.txt支持：解析和遵守网站的robots.txt规则
- [ ] 链接提取：从HTML中提取和跟踪链接
- [ ] 灵活配置：支持超时、重试、延迟等多种配置选项
- [ ] 代理支持：支持HTTP/HTTPS代理，包括基本认证
- [ ] URL过滤：支持自定义URL过滤器
- [ ] 内容处理：支持多个自定义内容处理回调函数
- [ ] 重定向处理：自动处理HTTP重定向

## 📝 实现步骤
1. ~~项目初始化：创建基础类 WebCrawler 并定义基本数据结构~~
2. ~~URL 管理：实现 URL 队列、去重功能。~~
3. ~~HTTP/HTTPS 请求：使用 httplib 进行网页爬取，并处理超时、重试等情况。~~
4. ~~数据存储：支持爬取结果的存取（如保存到数据库）。~~
5. HTML 解析：用 std::regex 提取网页中的超链接。
robots.txt 解析：确保遵守网站爬取规则。
6. ~~多线程支持：使用 std::thread 并发爬取，并同步数据。~~
7. 可扩展性：添加 URL 过滤、内容处理、代理支持等功能。
8. 优化与调试：改进线程管理，优化爬取效率。

## 反 反爬虫
- https://www.cnblogs.com/micro-chen/p/8676312.html
